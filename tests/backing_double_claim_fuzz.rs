...
/// Resolved winner holding TWO fully-occupied source domains on DIFFERENT
/// assets, each PARTIALLY backed (credit rate < 1): domain 0 = asset 0 long,
/// domain 2 = asset 1 long. 200 of claim each, backed by 150 and 100 atoms.
/// Upstream a4919ffa's own fixture.
fn resolved_market_with_two_underbacked_sources() -> (
    MarketGroupV16HeaderAccount,
    [Market<u64>; 2],
    PortfolioAccountV16Account,
) {
...
/// a4919ffa: a bounded resolved close must carry the UNCONVERTED source
/// haircut across calls as reserved (terminal-only) junior PnL. Without it the
/// first domain's haircut face looks like un-attributed positive PnL to the
/// account invariant "a source-claiming winner's positive PnL is entirely
/// source-attributed", so the very first `close_resolved` call reverts with
/// InvalidLeg and the winner's capital, its remaining backed claim and its
/// junior face are all stuck in the market with no instruction able to move
/// them. The retained face is also what stops the SECOND domain's backing from
/// buying face the FIRST domain already left behind.
#[test]
fn bounded_terminal_source_realization_preserves_intermediate_attribution() {
    let (mut header, mut markets, mut account_header) =
        resolved_market_with_two_underbacked_sources();
    let mut market = MarketGroupV16ViewMut::new(&mut header, &mut markets);
    let mut account = PortfolioV16ViewMut::new(&mut account_header);
    assert_eq!(market.validate_shape(), Ok(()));
    assert_eq!(account.validate_with_market(&market.as_view()), Ok(()));
